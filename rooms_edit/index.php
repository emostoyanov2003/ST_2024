...
<head>
    ...
    <title>Admin panel | Редактиране на стая</title>
...
<body>
    ...
    <div class="admin_page_container">
        <p class="admin_page_label">Редактиране на стая</p>
        <?php
            error_reporting(1);
            $id = $_GET["id"];
            $sqlSelectRow= "SELECT * FROM `rooms` WHERE MD5(`id`) = '$id'";
            ...
            echo '
                ...
                <input type="text" name="room_number" id="room_number" class="universal_input" placeholder="Номер на зала" autocomplete="off" value="' . $room_number . '">
                <p id="err_p1" class="err_p"></p>
                <input type="text" name="room_name" id="room_name" class="universal_input" placeholder="Име на зала" autocomplete="off" min="8" value="' . $room_name . '">
                <p id="err_p2" class="err_p"></p>
                    <div type="text" id="day_btn" class="input_dropdawn_fake_input" onmouseover="showDayDropdown();" onmouseout="hideDayDropdown();" style="color: #333;">Блок ' . $block . '</div>
                    <input type="text" name="block" id="day" class="hidden_input" value="' . $block . '">
                    ...
                    <input type="submit" name="submit" class="universal_submit_btn" value="Редактирай стая">
                </form>
            ';
            ...
            if(isset($_POST["submit"]) && $room_name != "" && $room_number != "" && $block != ""){
                ...
                if($conn->query($sqlInsert) === TRUE) {
                    if ($conn->affected_rows > 0) {
                        ...
                        $sqlDelete = "DELETE FROM `rooms` WHERE MD5(`id`) = '$id'";
