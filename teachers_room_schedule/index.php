...
<body>
    ...
    ?>
    <script>
        document.getElementById("teachers_room_schedule").style.borderLeft = "2px solid";
    </script>
    <div class="admin_page_container">
        <p class="admin_page_label">График на учителската стая</p>
        ...
        <button type="button" class="admin_page_control_btn" style="margin-bottom: 10px;" onclick="<?php echo "window.location.assign('../teachers_room_schedule_add');"?>"><i class="fa fa-plus-circle"></i> Добави</button>
        <script>
            <?php
                for ($i=0; $i < 12 * 7; $i+=7) { 
        ...
        <?php
            ...
            //Coloring
            $sql = "SELECT * FROM `teachers_room_schedule` WHERE `date` <= '$max_date' AND `date` >= '$min_date' ORDER BY `date` DESC";
            ...
            $sql = "SELECT * FROM `teachers_room_schedule` WHERE `owner_id` = '$owner_id' AND `date` <= '$max_date' AND `date` >= '$min_date' ORDER BY `date` DESC";
            $result = $conn -> query($sql);
            $number = 1;
            while($row = $result->fetch_assoc()){
                $id = $row["id"];
                ...
                echo '
                    <div class="admin_page_row">
                        ...
                        <p class="admin_page_row_p">' . $number . '. ' . $event_name . ' на ' . $date . ' от ' . $start_hour . ' ч. до ' . $end_hour . ' ч.</p>
                        <button type="button" class="admin_page_row_btn" onclick="window.location.assign(\'../teachers_room_schedule_edit/?id=' . md5($id) . '\')"><i class="fa fa-pencil"></i> Редактирай</button>
                    </div>
                ';
                $number++;
                if(isset($_POST["delete_marked_btn"])){
                    $sqlSelect = "SELECT * FROM `teachers_room_schedule`";
                    $result = $conn -> query($sqlSelect);
                    while($row = $result->fetch_assoc()){
                        $id = $row["id"];
                        if(isset($_POST["chackbox_" . $id  . ""])){
                            $sqlDelete = "DELETE FROM `teachers_room_schedule` WHERE `id` = '$id'";
